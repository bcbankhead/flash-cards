...
module.exports = {
 ...
 validateNewQuestion: function(question, correctAnswer, explanation, incorrectOne, incorrectTwo, incorrectThree){
   ...
   if(incorrectOne.trim() === "" || incorrectTwo.trim() === "" || incorrectThree.trim() === ""){
 ...
 ifAnswered: function(userAnswers, questions, callback){
   for (var i = 0; i < userAnswers.length; i++) {
     for (var j = 0; j < questions.length; j++) {
       if(questions[j]._id.toString() === userAnswers[i].questionId.toString() && userAnswers[i].answerPoints === '1'){
          questions[j].answered = 'true';
        } else if (questions[j]._id.toString() === userAnswers[i].questionId.toString() && userAnswers[i].answerPoints === '0'){
